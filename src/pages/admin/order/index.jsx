...
const Order = ({ isLoggedIn }) => {
  ...
  const fetchData = async () => {
    let url = '';
    // const roleCari = CekRole();

    try {
      // gak konsisten nilai nya, bener kalo saat dicari
      // if(role == 'merchant'){
      //   url = `http://localhost:5000/api/order?page=${currentPage}&role=merchant`
      // }
      // else{
      //   url = `http://localhost:5000/api/order?page=${currentPage}&role=` 
      // }

      const response = await axios.get(`http://localhost:5000/api/order?page=${currentPage}&role=${role}`);
  ...
  const fetchDataByKeyword = async (keyword) => {
    try {
      const response = await axios.get(
        `http://localhost:5000/api/order?keyword=${keyword}&role=${role}`
      );
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/api/order/${updateData.id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
